...
void draw() {
  println("Temperatura: " + temperatura + ", Humedad: " + humedad);
  if (puerto.available() > 0) {
    String lectura = puerto.readStringUntil('\n');
    if (lectura != null) {
      String[] datos = split(lectura.trim(), ' ');
      if (datos.length == 2) { // Verificar que hay dos elementos en el arreglo
        temperatura = parseFloat(datos[0]);
        humedad = parseFloat(datos[1]);
      }
    }
  }
